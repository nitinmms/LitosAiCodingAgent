...
/// through the exact same turn-execution path Program.cs already uses for typed text (see
/// BuildTurnContent) — never a bypassed/direct invocation, matching Claude Code's own prompt UX
/// where the server-authored text stands in for what the user would otherwise have typed.
/// Copied verbatim from Litos.Gui's McpPromptContentConverter per the Console parity plan's
/// no-Gui-changes rule. Deliberately not using ModelContextProtocol.AIContentExtensions.
/// ToChatMessages: that produces Microsoft.Extensions.AI.ChatMessage, a third message shape
/// unrelated to both MCP's own PromptMessage and Litos's own ChatMessage, which would need its
/// own second conversion into Litos.Agent.Messages.ContentBlock anyway.
/// </summary>
