...
/// source is either an image or goes through MarkItDown).
/// </summary>
public sealed record AttachResult(IReadOnlyList<string> Lines, DocumentMarkdown? Document, ImageBlock? Image);

/// <summary>
/// Shared by /attach and @mention handling (Program.cs): images bypass MarkItDown entirely and
/// go to the model as native vision content (ReadMe_AgentDesign.md §6.2) — MarkItDown's own
/// image handling only extracts EXIF metadata, which isn't visual content and was the root
/// cause of the model being unable to describe an attached image. A binary file MarkItDown has
/// no real converter for (anything not a recognized document format like PDF/DOCX — see
/// PlainTextMedia) is skipped entirely rather than sent to MarkItDown, which would otherwise
/// throw or, for some recognized-but-unintended extensions, misroute the file into a
/// structured-format sniffer that fails on it. Every other file type (plain text, source code,
/// or a real document format) keeps going through IAttachmentConverter as before.
/// </summary>
public sealed class AttachHandler(IAttachmentConverter converter)
{
    ...
    /// attachment and appends the confirmation line printed by /attach (but not by @mention,
    ...
    public async Task<AttachResult> AttachArgumentAsync(string argument, CancellationToken ct)
    {
        ...
        // Remote images still go through MarkItDown (EXIF/text only, no native vision) —
        // fetching and re-encoding remote image bytes for the model is a possible future
        // improvement, not covered by this change.
