...
/// /mcp__server__prompt command, split out so it's testable without an Avalonia control tree
/// (same reasoning as MainWindow.FindBranchPoints / CommandMenuPopup.Filter). Binding is
/// positional (Claude Code's own MCP prompt UX has no named/flag syntax to mirror), matching
/// each typed token in order against a prompt's declared PromptArguments.
/// </summary>
internal static class McpPromptArguments
{
    ...
    /// before pressing Enter in a single-line TextBox, so this stays lenient rather than
    ...
    /// <summary>
    /// Formats the "&lt;required&gt; [optional]" hint spliced into the transcript right after a
    /// prompt entry is picked from the command menu (see MainWindow.OnCommandMenuAcceptedAsync),
    /// in declared order. Empty/null Arguments yields "" — callers should skip printing a hint
    /// line entirely for a zero-argument prompt rather than show an empty one.
    /// </summary>
