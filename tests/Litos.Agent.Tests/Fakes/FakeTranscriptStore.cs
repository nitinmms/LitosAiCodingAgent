...
/// <summary>
/// In-memory ITranscriptStore keyed by (owner, sessionId). Records every AppendAsync call
/// in arrival order so tests can assert exactly what AgentLoop persisted and when.
/// </summary>
public sealed class FakeTranscriptStore : ITranscriptStore
{
    ...
    public List<TranscriptEntry> AppendedEntries { get; } = [];
    ...
    public async IAsyncEnumerable<TranscriptEntry> ReadAsync(SessionOwner owner, string sessionId, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct)
    {
    ...
    public Task<string> BranchAsync(SessionOwner owner, string sourceSessionId, int uptoEntryIndex, CancellationToken ct)
    {
        ...
        var source = _entries.TryGetValue((owner, sourceSessionId), out var list) ? list : [];
        var safeUpto = CompactionPlanner.SnapToSafeBranchPoint(source, uptoEntryIndex);
